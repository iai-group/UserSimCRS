...
class StructuredPreferenceModel(PreferenceModel):
    ...
    def _initialize_preferences(self) -> None:
        ...
        self._long_term_preferences.set_weighted_scores(
            [
                (
                    self._rating_preference_scores(),
                    self._rating_history_weight,
                ),
                (
                    self._dialogue_preference_scores(),
    ...
    def _rating_preference_scores(self) -> Dict[Tuple[str, str], List[float]]:
    ...
    def _dialogue_preference_scores(self) -> Dict[Tuple[str, str], List[float]]:
        ...
        for dialogue in self._dialogue_history:
            for utterance in self._user_utterances(dialogue):
                for signal in self._signals_extractor.extract(utterance):
                    if not signal.slot or signal.value is None:
                        continue
                    score = max(-1.0, min(1.0, signal.score))
                    dialogue_scores[(signal.slot, signal.value)].append(score)
    ...
    def update_from_utterance(self, utterance: Utterance) -> None:
        ...
        for signal in self._signals_extractor.extract(utterance):
            if not signal.slot or signal.value is None:
                continue
            score = max(-1.0, min(1.0, signal.score))
            self.update_slot_value_preference(signal.slot, signal.value, score)
