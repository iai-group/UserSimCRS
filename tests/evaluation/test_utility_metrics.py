...
def _annotated_utterance(
    ...
    return AnnotatedUtterance(
        ...
        dialogue_acts=[DialogueAct(Intent(i)) for i in intents],
    )
...
def test_success_rate_metric_rejection_overrides_acceptance() -> None:
    ...
    assert metric.evaluate_dialogue(dialogue) == 0.0
...
def test_reward_per_dialogue_length_counts_user_acceptances() -> None:
    ...
    assert metric.evaluate_dialogue(dialogue) == pytest.approx(1 / 2)
